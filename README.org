...
yay --noconfirm -Syu; and yay --noconfirm -Sc; and yay --noconfirm -Yc

# Install packages
yay -S ghostty fish starship zoxide emacs neovim yazi
...
yay -S ttf-jetbrains-mono ttf-jetbrains-mono-nerd noto-fonts-emoji
yay -S fzf fd ripgrep eza bat git-delta vivid 7zip plocate btop jaq htmlq age
yay -S rclone syncthing zrok-bin
yay -S zathura zathura-pdf-mupdf zathura-djvu
yay -S go gopls golangci-lint-bin pulumi
yay -S d2 pandoc-bin chromium pdfcpu
...
cd ~/.dotfiles; and ./config --init --all --install
#+END_SRC
...
#+BEGIN_SRC fish
# Install i3 packages
yay -S xorg-xev i3status-rust i3lock-color maim xclip
#+END_SRC

*** Configure font scaling and cursor size
...
# ~/.icons/default/cursors.conf
size=56
# ~/.config/gtk-3.0/settings.ini
[Settings]
gtk-cursor-theme-name=Bibata-Modern-Ice
gtk-cursor-theme-size=56
#+END_SRC
...
#+begin_src fish
yay -S mesa vulkan-intel intel-media-driver
yay -S pipeware wireplumber
yay -S xdg-desktop-portal xdg-desktop-portal-gnome
yay -S niri noctalia
yay -S emacs-wayland
#+end_src
...
| =z c=   | Fold (close)            |
| =z o=   | Unfold (open)           |
| =z a=   | Toggle fold (alternate) |
| =z m=   | Fold all (mute)         |
| =z r=   | Unfold all (reopen)     |
...
chmod 700 ~/.ssh; and chmod 600 ~/.ssh/id_*
...
zrok share reserved reservedShareToken --headless \
  --override-endpoint https://localhost:8081
...
#+begin_src fish
# Split djvu
set file 'Казка - Пан Коцький, 1983.djvu'
for i in (seq 1 (djvused "$file" -e 'n'))
  djvused "$file" -e "select $i; save-page-with page_$(printf '%02d' $i).djvu"
end
# Combine djvu
djvm -c "$file"  page_*.djvu
#+end_src
